...
# iterate through monthly files from January 2015 - December 2023
# 2024-2026 already provided from previous pull, so no need to repeat those years
year = 2015
month = 1
month_after = 2
year_after = 2015

while (year < 2023) or (year == 2023 and month <= 12):
    ...
    if token:
        ...
        params = {
            ...
            '$filter': f"MlsStatus eq 'Closed' and CloseDate ge {datetime(year, month, 1).isoformat(timespec='milliseconds')}Z and CloseDate lt {datetime(year_after, month_after, 1).isoformat(timespec='milliseconds')}Z",
